            ...
            foreach (var v in patronselector.m_AllFeatures)
            {
                ...
                if (v2 != null)
                {
                    ...
                    if (fixIcons)
                    {
                        patron.ModifyLevelEntries(x =>
                        {
                            foreach (var q in x.m_Features)
                            {
                                ...
                                Main.LotPContext.Logger.LogPatch($"Patched Icon on", feature);
                            }
                        });
                    }
                    if (fixWWProg)
                    {
                        ...
                        {
                            Main.LotPContext.Logger.LogPatch("Skipped Adding Winter Witch Progression to patron", v2);
                            hasWW = true;
                        }
                        ...
                        if (!hasWW)
                        {
                            Main.LotPContext.Logger.LogPatch("Added Winter Witch Progression to patron", v2);
